...
void autotune_output_func(double output) {
  ...
  // --- [START] AUTOTUNE REVERSE SERVO 修复 ---
  // Autotune 库总是假设 DIRECT (output MAX when below target).
  // 我们必须手动翻转它的继电器输出（1000/2000）以适配反向舵机。
  if (g_pid_reverse_action[g_autotune_servo_idx] == 1) {
    if (pwm_us == 1000.0) {
      pwm_us = 2000.0;
    } else if (pwm_us == 2000.0) {
      pwm_us = 1000.0;
    }
  }
  // --- [END] AUTOTUNE REVERSE SERVO 修复 ---
...
void start_autotune(int servo_index) {
  ...
  // --- [START] 编译错误修复 ---
  // 'PID' (来自 pid-autotune 库) 没有 .SetControllerDirection
  // 我们在 autotune_output_func 中处理反向逻辑
  if (g_pid_reverse_action[servo_index] == 1) {
    Serial.println("Autotune: Reverse servo detected. Logic will be inverted in output function.");
  } else {
    Serial.println("Autotune: Direct servo detected.");
  }
  // --- [END] 编译错误修复 ---
